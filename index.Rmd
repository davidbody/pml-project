...
The out-of-bag estimate of the error rate from the 10-fold cross-validation during training is about 1%, which is quite good.

## Out of Sample Error

Now we want to see how well our model performs on data that wasn't used to generate the model. We do this by generating predictions of `classe` on the testing data that we set aside earlier and comparing these predictions to the actual `classe` values in the testing data.
...
The confusion matrix allows us to easily compute the accuracy of our model on the testing data.

<p align="center" style="font-size: large">
$accuracy = \frac{number\ correct}{number\ correct + number\ incorrect}$
<p>

Our estimated model accuracy is therefore the sum of the numbers on the diagonal of the confusion matrix (the correct predictions) divided by the sum of all the numbers in the matrix (the total number of predictions). This is reported as Accuracy in the Overall Statistics above, and is about 99%. A 95% confidence interval for our model's accuracy is also reported.

Our out-of-sample error rate is simply
...
To gain some insights into our model, we can plot combinations of the top three variables against each other, and color the points based on our outcome variable.
...
These plots show some pretty tight groupings of observations, and a decent visible separation of the outcome values. Yet different outcomes are close enough together that additional variables beyond these three are likely to be important to the model's performance, as indicated by the output of varImp.
